...
- INPUT = string or numbers and command words;
- OUTPUT = number/s, but only if print command is issued, and multiple times if issued multiple times.
- RULES = input is a string, so need to different between words and numbers
  iterate through these, updating stack and register values as needed.
  only log if "print" command is given.
...
/*
string > array for input. iterating over array  and modifying stack [] and register integer.
*/
// TODO Algorithm
/*
function takes in string
  declare variable for string split into array by spaces
  loop over array
  if element number - register = number
  if element word - carry out appropriate function
    n, push, add, sub, etc...
  return
*/
// TODO Code
